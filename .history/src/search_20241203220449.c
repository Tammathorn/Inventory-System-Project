...
int show_list_product_struct(struct file_data *struct) {
    ...
    if ( (struct == NULL) ) {
        ...
        int i = 0; 
        while (struct[i].name != 0) {
            
            strcpy(name, struct[i].name);
            strcpy(type, struct[i].type);
            quantity = struct[i].quantity;
            price = struct[i].price;
